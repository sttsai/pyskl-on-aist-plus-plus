modality = 'bm'
graph = 'smpl24'
work_dir = './work_dirs_4F_13/dgstgcn/dgstgcn_aist++_smpl24_420/bm'
...
train_pipeline = [
    ...
    dict(type='ContinuousSampleDecode', clip_len=60),
...
val_pipeline = [
    dict(type='PreNormalize3D', align_spine=False),
    dict(type='GenSkeFeat', dataset=graph, feats=[modality]),
    dict(type='ContinuousSampleDecode', clip_len=60, num_clips=13),
    dict(type='FormatGCNInput', num_person=1),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['keypoint'])
]
