modality = 'bm'
graph = 'smpl24'
work_dir = './work_dirs_4C/dgstgcn/dgstgcn_aist++_smpl24_420/bm'
...
train_pipeline = [
    ...
    dict(type='UniformSampleDecode', clip_len=420),
...
val_pipeline = [
    dict(type='PreNormalize3D', align_spine=False),
    dict(type='GenSkeFeat', dataset=graph, feats=[modality]),
    dict(type='UniformSampleDecode', clip_len=420, num_clips=1),
    dict(type='FormatGCNInput', num_person=1),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['keypoint'])
]
test_pipeline = [
    dict(type='PreNormalize3D', align_spine=False),
    dict(type='GenSkeFeat', dataset=graph, feats=[modality]),
    dict(type='ContinuousSampleDecode', clip_len=60, num_clips=7),
    dict(type='FormatGCNInput', num_person=1),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['keypoint'])
]
