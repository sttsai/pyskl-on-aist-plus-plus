model = dict(
    ...
    backbone=dict(
        type='MSG3D',
        graph_cfg=dict(layout='smpl24', mode='binary_adj'),
        num_person=1),
    cls_head=dict(type='GCNHead', num_classes=10, in_channels=384))

dataset_type = 'PoseDataset'
ann_file = 'data/aist++/aist++_smpl24_360.pkl'
train_pipeline = [
    dict(type='PreNormalize3D'),
    dict(type='GenSkeFeat', dataset='smpl24', feats=['bm']),
    dict(type='UniformSample', clip_len=360),
...
val_pipeline = [
    dict(type='PreNormalize3D'),
    dict(type='GenSkeFeat', dataset='smpl24', feats=['bm']),
    dict(type='UniformSample', clip_len=360, num_clips=1),
    dict(type='PoseDecode'),
    dict(type='FormatGCNInput', num_person=1),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['keypoint'])
...
# runtime settings
log_level = 'INFO'
work_dir = './work_dirs_4B/msg3d/msg3d_aist++_smpl24_360/bm'